...
//**************以下TIM1实现超声波测距****************//
u16 TIM1CH4_CAPTURE_STA,TIM1CH4_CAPTURE_VAL;
...
void TIM1_Cap_Init(u16 arr,u16 psc)
{
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;     //PA8输出 
	...
	TIM_ICInitStructure.TIM_Channel =  TIM_Channel_4; //IC1映射到TI1上，这四句不能合并
	...
	TIM_ITConfig(TIM1,TIM_IT_Update|TIM_IT_CC4,ENABLE);   //允许更新中断，允许CC1IE,CC2IE,CC3IE,CC4IE捕获中断	
...
{   
	...
	if(TIM1CH4_CAPTURE_STA&0X80)
	{
		...
		OLED_ShowFloat(90,47,(float)Distance,5,12,1);
//		printf("%d \r\n",Distance);
...
void TIM1_CC_IRQHandler(void)
{
	if((TIM1CH4_CAPTURE_STA&0X80) == 0)
	{
		...
		{	
			...
			{
				TIM1CH4_CAPTURE_STA|=0X80;	
				TIM1CH4_CAPTURE_VAL = TIM_GetCapture4(TIM1);//记录下此时的定时器计数值
